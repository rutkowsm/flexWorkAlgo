...
class Vacancy:
    def __init__(self, day, start_time, end_time):
        ...
        self.blocks = ['EMPTY'] * (end_time - start_time + 1)
...
def swap_shift(current_employee, new_employee, day_index, block_index, vacancy):
    ...
    for hour in range(block_index, block_index + current_employee.min_shift_length):
        ...
        index = hour - vacancy.start_time  # Calculate the correct index for vacancy.blocks
        if 0 <= index < len(vacancy.blocks):
            vacancy.blocks[index] = new_employee.name
...
employee_2 = Employee(name="Kate", email="kate@mail.com", personal_calendar=kate_calendar, min_shift_length=4,
                      max_shift_length=10)
employee_3 = Employee(name="Bill", email="bill@mail.com", personal_calendar=bill_calendar, min_shift_length=4,
                      max_shift_length=10)

employees = [employee_1, employee_2, employee_3]

restaurant = Restaurant("Bar Mleczny Apis")
restaurant.add_vacancy(1, 8, 16)  # Add a vacancy on Tue from 8:00 to 16:00
restaurant.add_vacancy(2, 10, 17)  # Add vacancy on Wed from 10:00 to 17:00
restaurant.add_vacancy(3, 10, 18)  # Add vacancy on Thu from 12:00 to 16:00
restaurant.add_vacancy(4, 10, 18)  # Add vacancy on Fri from 12:00 to 16:00
restaurant.add_vacancy(4, 12, 18)  # Add vacancy on Fri from 12:00 to 16:00
restaurant.add_vacancy(5, 10, 18)  # Add vacancy on Sat from 12:00 to 16:00
restaurant.add_vacancy(5, 12, 18)  # Add vacancy on Sat from 12:00 to 16:00
restaurant.add_vacancy(5, 12, 16)  # Add vacancy on Sat from 12:00 to 16:00
restaurant.add_vacancy(6, 10, 18)  # Add vacancy on Sat from 12:00 to 16:00
restaurant.add_vacancy(6, 12, 18)  # Add vacancy on Sat from 12:00 to 16:00
...
for day_index, vacancies in enumerate(restaurant_schedule.restaurant_calendar):
    ...
    print(f"Day {day_index} - {day_of_week}:")

    # Check if there are any vacancies for this day
    if vacancies:
        for vacancy in vacancies:
            print(
